...
The app now shows up correctly in my browser at localhost:8080. But it doesn't show all items added so far after a client enters a new item. (This seems to be the intention of the developer as she/he made a query to get all items in the logic handling a successful item submit). So I added some code to loop over the query result and made the server return all items addes thus far in a list. 
